...
if [ $# -lt 1 ]
then
	echo "必须传入all/表名..., 第二参数为统计日期可选，默认为昨天"
	exit
fi
...
from (select order_id,
      ...
      where dt = '${datestr}'
      group by order_id, receiver_district_id) detail
         left join
     (select id        org_id,
             org_name,
...
case $1 in
"all")
	/export/server/hive/bin/hive -e "use tms;${dws_trade_org_cargo_type_order_1d_sql}${dws_trans_dispatch_1d_sql}${dws_trans_org_deliver_suc_1d_sql}${dws_trans_org_receive_1d_sql}${dws_trans_org_sort_1d_sql}${dws_trans_org_truck_model_type_trans_finish_1d_sql}"
;;
"dws_trade_org_cargo_type_order_1d")
    /export/server/hive/bin/hive -e "use tms;${dws_trade_org_cargo_type_order_1d_sql}"
;;
"dws_trans_dispatch_1d")
    /export/server/hive/bin/hive -e "use tms;${dws_trans_dispatch_1d_sql}"
;;
"dws_trans_org_deliver_suc_1d")
    /export/server/hive/bin/hive -e "use tms;${dws_trans_org_deliver_suc_1d_sql}"
;;
"dws_trans_org_receive_1d")
    /export/server/hive/bin/hive -e "use tms;${dws_trans_org_receive_1d_sql}"
;;
"dws_trans_org_sort_1d")
    /export/server/hive/bin/hive -e "use tms;${dws_trans_org_sort_1d_sql}"
;;
"dws_trans_org_truck_model_type_trans_finish_1d")
    /export/server/hive/bin/hive -e "use tms;${dws_trans_org_truck_model_type_trans_finish_1d_sql}"
;;
*)
	echo "表名输入错误..."
;;
