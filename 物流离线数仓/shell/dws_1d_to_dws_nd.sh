...
case $1 in
"all")
	/export/server/hive/bin/hive -e "use tms;${dws_trans_shift_trans_finish_nd_sql}${dws_trade_org_cargo_type_order_nd_sql}${dws_trans_org_sort_nd_sql}${dws_trans_dispatch_nd_sql}${dws_trans_org_receive_nd_sql}${dws_trans_org_deliver_suc_nd_sql}"
;;
"dws_trade_org_cargo_type_order_nd")
    /export/server/hive/bin/hive -e "use tms;${dws_trade_org_cargo_type_order_nd_sql}"
;;
"dws_trans_dispatch_nd")
    /export/server/hive/bin/hive -e "use tms;${dws_trans_dispatch_nd_sql}"
;;
"dws_trans_org_deliver_suc_nd")
    /export/server/hive/bin/hive -e "use tms;${dws_trans_org_deliver_suc_nd_sql}"
;;
"dws_trans_org_receive_nd")
    /export/server/hive/bin/hive -e "use tms;${dws_trans_org_receive_nd_sql}"
;;
"dws_trans_org_sort_nd")
    /export/server/hive/bin/hive -e "use tms;${dws_trans_org_sort_nd_sql}"
;;
"dws_trans_shift_trans_finish_nd")
    /export/server/hive/bin/hive -e "use tms;${dws_trans_shift_trans_finish_nd_sql}"
;;
*)
	echo "表名输入错误..."
;;
