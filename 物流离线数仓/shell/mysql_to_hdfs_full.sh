...
DATAX_HOME=/export/server/datax
DATAX_DATA=/export/server/datax/job
...
base_complex | base_dic | base_region_info | base_organ | express_courier | express_courier_complex | employee_info | line_base_shift | line_base_info | truck_driver | truck_info | truck_model | truck_team)
  import_data $DATAX_DATA/tms/import/tms.${tab}.json /tms_origin_data/${tab}_full/$do_date
  ;;
"all")
  ...
  do
    import_data $DATAX_DATA/tms/import/tms.${tmp}.json /tms_origin_data/${tmp}_full/$do_date
  done
  ;;
esac
