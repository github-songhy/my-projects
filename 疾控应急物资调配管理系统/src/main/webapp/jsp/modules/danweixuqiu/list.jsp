...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">单位需求管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>单位需求列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">单位需求列表</h3>
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									<label>
										单位负责人
									</label>
									<input type="text" id="dwfzrxingmingSearch" class="form-control form-control-sm" placeholder="请输入姓名" aria-controls="tableId">
									<label>
										需求物资
									</label>
									<select name="xuqiuwuzi" id="xuqiuwuziSelect" class="form-control form-control-sm" aria-controls="tableId">
									</select>
                                    <label>
                                        是否通过
                                    </label>
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('danweizhanghao')">单位账号<i id="danweizhanghaoIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('shoujihaoma')">手机号码<i id="shoujihaomaIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('dwfzrxingming')">单位负责人姓名<i id="dwfzrxingmingIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('danweidizhi')">单位地址<i id="danweidizhiIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('xuqiuwuzi')">需求物资<i id="xuqiuwuziIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('xuqiushuliang')">需求数量<i id="xuqiushuliangIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			if($('#dwfzrxingmingSearch').val() != null && $('#dwfzrxingmingSearch').val() != ''){
				searchForm.dwfzrxingming ="%" +  $('#dwfzrxingmingSearch').val() + "%";
			}
			
			var xuqiuwuziIndex = document.getElementById("xuqiuwuziSelect").selectedIndex;
			var xuqiuwuziOptions = document.getElementById("xuqiuwuziSelect").options;
			var xuqiuwuziSelectedOption = xuqiuwuziOptions[xuqiuwuziIndex]
			if(xuqiuwuziSelectedOption.text != null && xuqiuwuziSelectedOption.text != ''){
				searchForm.xuqiuwuzi = xuqiuwuziSelectedOption.text;
			}
        ...
        function getDataList() {
			http("danweixuqiu/page","GET",{
				...
				sort: sortColumn,
				order: sortOrder,
				dwfzrxingming : searchForm.dwfzrxingming,
				xuqiuwuzi : searchForm.xuqiuwuzi,	
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var danweizhanghaoCell  = document.createElement('td');

						 danweizhanghaoCell.innerHTML = item.danweizhanghao;

			row.appendChild(danweizhanghaoCell);
			var shoujihaomaCell  = document.createElement('td');

						 shoujihaomaCell.innerHTML = item.shoujihaoma;

			row.appendChild(shoujihaomaCell);
			var dwfzrxingmingCell  = document.createElement('td');

						 dwfzrxingmingCell.innerHTML = item.dwfzrxingming;

			row.appendChild(dwfzrxingmingCell);
			var danweidizhiCell  = document.createElement('td');

						 danweidizhiCell.innerHTML = item.danweidizhi;

			row.appendChild(danweidizhiCell);
			var xuqiuwuziCell  = document.createElement('td');

						 xuqiuwuziCell.innerHTML = item.xuqiuwuzi;

			row.appendChild(xuqiuwuziCell);
			var xuqiushuliangCell  = document.createElement('td');

						 xuqiushuliangCell.innerHTML = item.xuqiushuliang;

			row.appendChild(xuqiushuliangCell);
		...
		function sh(){
			...
			httpJson("danweixuqiu/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("danweixuqiu/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"danweixuqiu");
